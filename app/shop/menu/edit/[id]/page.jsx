...
export default function EditMenuPage() {
  ...
  const [shouldRemoveImg, setShouldRemoveImg] = useState(false)
  ...
  async function save() {
    ...
    try {
      ...
      fd.append('usrId', JSON.parse(localStorage.getItem('bs_user')).id)
      fd.append('removeImage', shouldRemoveImg ? 'true' : 'false')
      if (imgFile) fd.append('image', imgFile)

      const res = await fetch(`http://localhost/bitesync/api/shop/menu.php?id=${params.id}`, {
        method: 'POST', // Use POST for multipart/form-data support
        headers: {
      ...
      const data = await res.json()
      if (data.success) { toast_('แก้ไขเมนูสำเร็จ!') }
      ...
      <div className={styles.layout}>
        ...
        <div className={styles.right}>
          <div className={`${styles.previewCard} ${preview ? styles.previewActive : ''}`} 
               onClick={preview ? () => {
                 const src = originalImg || preview;
                 const url = src.startsWith('blob:') ? src : `http://localhost/bitesync/api/shop/image_proxy.php?file=${src}`;
                 setImageToCrop(url);
               } : null}>
            {preview
              ? <><img src={preview.startsWith('blob:') ? preview : `http://localhost/bitesync/public${preview}`} className={styles.previewImg}/><div className={styles.previewOver}>✎ แก้ไขรูปภาพ</div></>
