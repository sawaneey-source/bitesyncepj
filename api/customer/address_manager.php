...
elseif ($method === 'POST') {
    ...
    // 1. Check if EXACT match already exists (Exclude soft-deleted ones)
    $chk = $conn->prepare("SELECT AdrId FROM tbl_address WHERE UsrId = ? AND HouseNo = ? AND SubDistrict = ? AND District = ? AND Province = ? AND AdrLat = ? AND AdrLng = ? AND IsDefault != 2 LIMIT 1");
    ...
    if ($existing) {
        ...
        if ($isDef) {
            $conn->query("UPDATE tbl_address SET IsDefault = 0 WHERE UsrId = " . (int)$userId . " AND IsDefault = 1");
